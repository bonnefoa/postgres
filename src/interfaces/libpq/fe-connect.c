...
pqConnectOptions2(PGconn *conn)
{
	...
	/*
	 * A Pfdebug may have been copied from a previous connection, use it in
	 * this case
	 */
	if (conn->trace_file_str && !conn->Pfdebug)
	{
		...
		{
			/*
			 * if the connection is created for a cancel request, don't try to
			 * trace it as the source connection has already an opened fd on
			 * this trace file
			 */
			if (!conn->cancelRequest)
			{
...
freePGconn(PGconn *conn)
{
	 ...
	 * externally. If tracing was enabled through PGTRACE, conn->Pfdebug will
	 * be set to conn->trace_file, so closing conn->trace_file is enough.
	 */
