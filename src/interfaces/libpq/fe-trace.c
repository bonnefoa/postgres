...
pqTraceOutputMessage(PGconn *conn, const char *message, bool toServer)
{
	...
	if ((conn->traceFlags & PQTRACE_SUPPRESS_TIMESTAMPS) == 0)
	{
		...
		appendPQExpBuffer(messagebuf, "%s\t", timestr);
	}
	...
	if (regress && !toServer && (id == PqMsg_ErrorResponse
		...
		appendPQExpBuffer(messagebuf, "%s\tNN\t", prefix);
	else
		appendPQExpBuffer(messagebuf, "%s\t%d\t", prefix, length);

	switch (id)
	{
		case PqMsg_ParseComplete:
			appendPQExpBufferStr(messagebuf, "ParseComplete");
			/* No message content */
			break;
		case PqMsg_BindComplete:
			appendPQExpBufferStr(messagebuf, "BindComplete");
			/* No message content */
			break;
		case PqMsg_CloseComplete:
			appendPQExpBufferStr(messagebuf, "CloseComplete");
			/* No message content */
			break;
		case PqMsg_NotificationResponse:
			pqTraceOutput_NotificationResponse(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_Bind:
			pqTraceOutput_Bind(messagebuf, message, &logCursor);
			break;
		case PqMsg_CopyDone:
			appendPQExpBufferStr(messagebuf, "CopyDone");
			/* No message content */
			break;
		case PqMsg_CommandComplete:
			...
			if (toServer)
				pqTraceOutput_Close(messagebuf, message, &logCursor);
			else
				pqTraceOutput_CommandComplete(messagebuf, message, &logCursor);
			break;
		case PqMsg_CopyData:
			pqTraceOutput_CopyData(messagebuf, message, &logCursor,
								   length, regress);
			break;
		case PqMsg_Describe:
			...
			if (toServer)
				pqTraceOutput_Describe(messagebuf, message, &logCursor);
			else
				pqTraceOutput_DataRow(messagebuf, message, &logCursor);
			break;
		case PqMsg_Execute:
			/* Execute(F) and ErrorResponse(B) use the same identifier. */
			Assert(PqMsg_Execute == PqMsg_ErrorResponse);
			if (toServer)
				pqTraceOutput_Execute(messagebuf, message, &logCursor, regress);
			else
				pqTraceOutput_ErrorResponse(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_CopyFail:
			pqTraceOutput_CopyFail(messagebuf, message, &logCursor);
			break;
		case PqMsg_GSSResponse:
			...
			switch (conn->current_auth_response)
			{
				case AUTH_RESPONSE_GSS:
					pqTraceOutput_GSSResponse(messagebuf, message,
											  &logCursor, length, regress);
					break;
				case AUTH_RESPONSE_PASSWORD:
					pqTraceOutput_PasswordMessage(messagebuf, message,
												  &logCursor);
					break;
				case AUTH_RESPONSE_SASL_INITIAL:
					pqTraceOutput_SASLInitialResponse(messagebuf, message,
													  &logCursor, regress);
					break;
				case AUTH_RESPONSE_SASL:
					pqTraceOutput_SASLResponse(messagebuf, message,
											   &logCursor, length, regress);
					break;
				default:
					appendPQExpBufferStr(messagebuf, "UnknownAuthenticationResponse");
					break;
			}
			conn->current_auth_response = '\0';
			break;
		case PqMsg_FunctionCall:
			pqTraceOutput_FunctionCall(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_CopyInResponse:
			pqTraceOutput_CopyInResponse(messagebuf, message, &logCursor);
			break;
		case PqMsg_Flush:
			/* Flush(F) and CopyOutResponse(B) use the same identifier */
			Assert(PqMsg_CopyOutResponse == PqMsg_Flush);
			if (toServer)
				appendPQExpBufferStr(messagebuf, "Flush");	/* no message content */
			else
				pqTraceOutput_CopyOutResponse(messagebuf, message, &logCursor);
			break;
		case PqMsg_EmptyQueryResponse:
			appendPQExpBufferStr(messagebuf, "EmptyQueryResponse");
			/* No message content */
			break;
		case PqMsg_BackendKeyData:
			pqTraceOutput_BackendKeyData(messagebuf, message, &logCursor,
										 length, regress);
			break;
		case PqMsg_NoData:
			appendPQExpBufferStr(messagebuf, "NoData");
			/* No message content */
			break;
		case PqMsg_NoticeResponse:
			pqTraceOutput_NoticeResponse(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_Parse:
			pqTraceOutput_Parse(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_Query:
			pqTraceOutput_Query(messagebuf, message, &logCursor);
			break;
		case PqMsg_AuthenticationRequest:
			pqTraceOutput_Authentication(messagebuf, message, &logCursor,
										 length, regress);
			break;
		case PqMsg_PortalSuspended:
			appendPQExpBufferStr(messagebuf, "PortalSuspended");
			/* No message content */
			break;
		case PqMsg_Sync:
			...
			if (toServer)
				appendPQExpBufferStr(messagebuf, "Sync");	/* no message content */
			else
				pqTraceOutput_ParameterStatus(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_ParameterDescription:
			pqTraceOutput_ParameterDescription(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_RowDescription:
			pqTraceOutput_RowDescription(messagebuf, message, &logCursor, regress);
			break;
		case PqMsg_NegotiateProtocolVersion:
			pqTraceOutput_NegotiateProtocolVersion(messagebuf, message, &logCursor);
			break;
		case PqMsg_FunctionCallResponse:
			pqTraceOutput_FunctionCallResponse(messagebuf, message, &logCursor);
			break;
		case PqMsg_CopyBothResponse:
			pqTraceOutput_CopyBothResponse(messagebuf, message, &logCursor, length);
			break;
		case PqMsg_Terminate:
			appendPQExpBufferStr(messagebuf, "Terminate");
			/* No message content */
			break;
		case PqMsg_ReadyForQuery:
			pqTraceOutput_ReadyForQuery(messagebuf, message, &logCursor);
			break;
		default:
			appendPQExpBuffer(messagebuf, "Unknown message: %02x", id);
			break;
	}

	appendPQExpBufferChar(messagebuf, '\n');
	...
	if (logCursor - 1 != length)
		appendPQExpBuffer(messagebuf,
						  "mismatched message length: consumed %d, expected %d\n",
						  logCursor - 1, length);

	fprintf(conn->Pfdebug, "%s", messagebuf->data);
	resetPQExpBuffer(messagebuf);
}
...
pqTraceOutputNoTypeByteMessage(PGconn *conn, const char *message)
{
	...
	if (length < 8)
	{
		appendPQExpBuffer(messagebuf, "F\t%d\tUnknown message\n", length);
		fprintf(conn->Pfdebug, "%s", messagebuf->data);
		resetPQExpBuffer(messagebuf);
		return;
	}
	...
	if (regress && (version != CANCEL_REQUEST_CODE
		...
		appendPQExpBufferStr(messagebuf, "F\tNN\t");
	else
		appendPQExpBuffer(messagebuf, "F\t%d\t", length);

	if (version == CANCEL_REQUEST_CODE && length >= 16)
	{
		appendPQExpBufferStr(messagebuf, "CancelRequest\t");
		pqTraceOutputInt16(messagebuf, message, &logCursor);
		pqTraceOutputInt16(messagebuf, message, &logCursor);
		pqTraceOutputInt32(messagebuf, message, &logCursor, regress);
		pqTraceOutputNbyte(messagebuf, length - logCursor, message,
						   &logCursor, regress);
	}
	else if (version == NEGOTIATE_SSL_CODE)
	{
		appendPQExpBufferStr(messagebuf, "SSLRequest\t");
		pqTraceOutputInt16(messagebuf, message, &logCursor);
		pqTraceOutputInt16(messagebuf, message, &logCursor);
	}
	else if (version == NEGOTIATE_GSS_CODE)
	{
		appendPQExpBufferStr(messagebuf, "GSSENCRequest\t");
		pqTraceOutputInt16(messagebuf, message, &logCursor);
		pqTraceOutputInt16(messagebuf, message, &logCursor);
	}
	else
	{
		appendPQExpBufferStr(messagebuf, "StartupMessage\t");
		pqTraceOutputInt16(messagebuf, message, &logCursor);
		pqTraceOutputInt16(messagebuf, message, &logCursor);
		while (message[logCursor] != '\0')
		{
			pqTraceOutputString(messagebuf, message, &logCursor, false);
			pqTraceOutputString(messagebuf, message, &logCursor, regress);
		}
						  ...
						  char response)
{
	...
	appendPQExpBuffer(messagebuf, "B\t1\t%s\t %c\n", responseType, response);
